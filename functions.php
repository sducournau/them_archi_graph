...
function archi_enqueue_scripts() {
    ...
    // Graph particles background effect - DÉSACTIVÉ
    /*
    wp_enqueue_style(
        'archi-graph-particles',
        ARCHI_THEME_URI . '/assets/css/graph-particles.css',
        ['archi-graph-effects'],
        ARCHI_THEME_VERSION
    );
    */
    
    // Graph category polygons enhancements
    wp_enqueue_style(
        'archi-graph-polygons',
        ARCHI_THEME_URI . '/assets/css/graph-polygons.css',
        ['archi-graph-effects'],
        ARCHI_THEME_VERSION
    );
    ...
    if (is_front_page()) {
        ...
        // 🔥 DESTRUCTION TOTALE DU CACHE - Change le nom du fichier lui-même
        $cache_bust = time() . wp_rand(1000, 9999);
        
        wp_enqueue_script(
            'archi-vendors',
            add_query_arg('v', $cache_bust, ARCHI_THEME_URI . '/dist/js/vendors.bundle.js'),
            ['archi-graph-init-early'],
            null, // Pas de version, on l'a dans l'URL
            true
        );
        
        wp_enqueue_script(
            'archi-app',
            add_query_arg('v', $cache_bust, ARCHI_THEME_URI . '/dist/js/app.bundle.js'),
            ['archi-vendors'],
            null, // Pas de version, on l'a dans l'URL
            true
        );
        
        // Ambient particles effect for graph background - DÉSACTIVÉ
        /*
        wp_enqueue_script(
            'archi-graph-ambient-particles',
            ARCHI_THEME_URI . '/assets/js/graph-ambient-particles.js',
            [],
            ARCHI_THEME_VERSION,
            true
        );
    ...
    if (is_singular() || is_page_template('page-guestbook.php')) {
        wp_enqueue_style(
            'archi-feedback-system',
            ARCHI_THEME_URI . '/assets/css/feedback-system.css',
            [],
            ARCHI_THEME_VERSION
        );
    }
    ...
    if (is_front_page()) {
        ...
        wp_localize_script('archi-app', 'archiGraphSettings', [
            ...
            // 🔥 NEW: Node halo effect
            'nodeHaloEnabled' => get_theme_mod('archi_node_halo_enabled', true),
            'nodeHaloWidth' => get_theme_mod('archi_node_halo_width', 3),
            'nodeHaloOpacity' => get_theme_mod('archi_node_halo_opacity', 0.5),
            ...
            // Effects preset
            'effectsPreset' => get_theme_mod('archi_effects_preset', 'normal'),
            
            // Active node glow
            'activeNodeGlowEnabled' => get_theme_mod('archi_active_node_glow_enabled', true),
            'activeNodeGlowIntensity' => get_theme_mod('archi_active_node_glow_intensity', 25),
            'activeNodeGlowOpacity' => get_theme_mod('archi_active_node_glow_opacity', 0.8),
            
            // Node shadows
            'nodeShadowEnabled' => get_theme_mod('archi_node_shadow_enabled', true),
            'nodeShadowBlur' => get_theme_mod('archi_node_shadow_blur', 6),
            'nodeShadowOpacity' => get_theme_mod('archi_node_shadow_opacity', 0.3),
            
            // Pulse animation
            'nodePulseEnabled' => get_theme_mod('archi_node_pulse_enabled', true),
            'nodePulseDuration' => get_theme_mod('archi_node_pulse_duration', 2500),
            'nodePulseIntensity' => get_theme_mod('archi_node_pulse_intensity', 0.85),
            
            // Floating particles
            'particlesEnabled' => get_theme_mod('archi_particles_enabled', true),
            'particlesCount' => get_theme_mod('archi_particles_count', 20),
            'particlesOpacity' => get_theme_mod('archi_particles_opacity', 0.15),
            'particlesSpeed' => get_theme_mod('archi_particles_speed', 15),
            
            // Ambient glow
            'ambientGlowEnabled' => get_theme_mod('archi_ambient_glow_enabled', true),
            'ambientGlowOpacity' => get_theme_mod('archi_ambient_glow_opacity', 0.3),
            'ambientGlowDuration' => get_theme_mod('archi_ambient_glow_duration', 8),
            
            // Hover effects
            'hoverScale' => get_theme_mod('archi_hover_scale', 1.2),
            'hoverTransitionDuration' => get_theme_mod('archi_hover_transition_duration', 300),
            'hoverBrightness' => get_theme_mod('archi_hover_brightness', 1.15),
            
            // Active node (selected)
            'activeNodeScaleEffect' => get_theme_mod('archi_active_node_scale', 1.5),
            'activeNodeGlowAnimation' => get_theme_mod('archi_active_node_glow_animation', 'pulse')
        ]);
...
// Graph configuration registry (MUST be loaded first)
...
function archi_render_social_links() {
    ...
    $icons = [
        'facebook' => '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 96 96" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>',
        'twitter' => '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 96 96" fill="currentColor"><path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/></svg>',
        'linkedin' => '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 96 96" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>',
        'instagram' => '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 96 96" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>',
        'github' => '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 96 96" fill="currentColor"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg>',
        'youtube' => '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 96 96" fill="currentColor"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>'
    ];
...
/**
 * 🔥 NOUVEAU: Injecter les variables CSS pour les effets visuels depuis le Customizer
 * Permet de contrôler les animations, lueurs, ombres, particules en temps réel
 * 
 * @since 2.0.0
 */
function archi_inject_visual_effects_css_vars() {
    // Récupérer tous les paramètres d'effets visuels
    $preset = get_theme_mod('archi_effects_preset', 'normal');
    
    // Active node glow
    $active_glow_enabled = get_theme_mod('archi_active_node_glow_enabled', true);
    $active_glow_intensity = get_theme_mod('archi_active_node_glow_intensity', 25);
    $active_glow_opacity = get_theme_mod('archi_active_node_glow_opacity', 0.8);
    
    // Node shadows
    $shadow_enabled = get_theme_mod('archi_node_shadow_enabled', true);
    $shadow_blur = get_theme_mod('archi_node_shadow_blur', 6);
    $shadow_opacity = get_theme_mod('archi_node_shadow_opacity', 0.3);
    
    // Pulse animation
    $pulse_enabled = get_theme_mod('archi_node_pulse_enabled', true);
    $pulse_duration = get_theme_mod('archi_node_pulse_duration', 2500);
    $pulse_intensity = get_theme_mod('archi_node_pulse_intensity', 0.85);
    
    // Particles
    $particles_enabled = get_theme_mod('archi_particles_enabled', true);
    $particles_count = get_theme_mod('archi_particles_count', 20);
    $particles_opacity = get_theme_mod('archi_particles_opacity', 0.15);
    $particles_speed = get_theme_mod('archi_particles_speed', 15);
    
    // Ambient glow
    $ambient_glow_enabled = get_theme_mod('archi_ambient_glow_enabled', true);
    $ambient_glow_opacity = get_theme_mod('archi_ambient_glow_opacity', 0.3);
    $ambient_glow_duration = get_theme_mod('archi_ambient_glow_duration', 8);
    
    // Hover effects
    $hover_scale = get_theme_mod('archi_hover_scale', 1.2);
    $hover_transition = get_theme_mod('archi_hover_transition_duration', 300);
    $hover_brightness = get_theme_mod('archi_hover_brightness', 1.15);
    
    // Active node
    $active_scale = get_theme_mod('archi_active_node_scale', 1.5);
    $active_animation = get_theme_mod('archi_active_node_glow_animation', 'pulse');
    
    // Générer le CSS avec variables
    echo '<style id="archi-visual-effects-vars">
        :root {
            /* 🔥 Active Node Glow */
            --archi-active-glow-enabled: ' . ($active_glow_enabled ? '1' : '0') . ';
            --archi-active-glow-intensity: ' . absint($active_glow_intensity) . 'px;
            --archi-active-glow-opacity: ' . floatval($active_glow_opacity) . ';
            --archi-active-glow-blur: ' . (absint($active_glow_intensity) / 8) . 'px;
            
            /* 🔥 Node Shadows */
            --archi-shadow-enabled: ' . ($shadow_enabled ? '1' : '0') . ';
            --archi-shadow-blur: ' . absint($shadow_blur) . 'px;
            --archi-shadow-opacity: ' . floatval($shadow_opacity) . ';
            
            /* 🔥 Pulse Animation */
            --archi-pulse-enabled: ' . ($pulse_enabled ? '1' : '0') . ';
            --archi-pulse-duration: ' . absint($pulse_duration) . 'ms;
            --archi-pulse-intensity: ' . floatval($pulse_intensity) . ';
            
            /* 🔥 Particles */
            --archi-particles-enabled: ' . ($particles_enabled ? '1' : '0') . ';
            --archi-particles-count: ' . absint($particles_count) . ';
            --archi-particles-opacity: ' . floatval($particles_opacity) . ';
            --archi-particles-speed: ' . absint($particles_speed) . 's;
            
            /* 🔥 Ambient Glow */
            --archi-ambient-glow-enabled: ' . ($ambient_glow_enabled ? '1' : '0') . ';
            --archi-ambient-glow-opacity: ' . floatval($ambient_glow_opacity) . ';
            --archi-ambient-glow-duration: ' . absint($ambient_glow_duration) . 's;
            
            /* 🔥 Hover Effects */
            --archi-hover-scale: ' . floatval($hover_scale) . ';
            --archi-hover-transition: ' . absint($hover_transition) . 'ms;
            --archi-hover-brightness: ' . floatval($hover_brightness) . ';
            
            /* 🔥 Active Node */
            --archi-active-scale: ' . floatval($active_scale) . ';
            --archi-active-animation: ' . esc_attr($active_animation) . ';
        }
        
        /* 🔥 Appliquer les effets conditionnels via CSS */
        ' . (!$particles_enabled ? '.floating-particles { display: none !important; }' : '') . '
        ' . (!$pulse_enabled ? '.node-pulse, .shine-pulse { animation: none !important; }' : '') . '
        ' . (!$ambient_glow_enabled ? '.ambient-glow { opacity: 0 !important; }' : '') . '
    </style>';
}
add_action('wp_head', 'archi_inject_visual_effects_css_vars', 20);
