...
function archi_add_meta_boxes() {
    ...
    foreach ($post_types as $post_type) {
        ...
        // Meta box pour l'image mise en avant
        add_meta_box(
            'archi-featured-image-settings',
            __('Options Image Mise en Avant', 'archi-graph'),
            'archi_featured_image_meta_box_callback',
            $post_type,
            'side',
            'low'
        );
...
/**
 * Callback pour la meta box des options d'image mise en avant
 */
function archi_featured_image_meta_box_callback($post) {
    // Nonce pour la sécurité
    wp_nonce_field('archi_featured_image_meta_box', 'archi_featured_image_meta_box_nonce');
    ...
    <table class="form-table">
        <tr>
            <td>
                <label for="archi_featured_image_fullscreen">
                    <input type="checkbox" 
                           id="archi_featured_image_fullscreen" 
                           name="archi_featured_image_fullscreen" 
                           value="1" 
                           <?php checked($fullscreen, '1'); ?>>
                    <?php _e('🖼️ Afficher en plein écran', 'archi-graph'); ?>
                </label>
                <p class="description">
        ...
        <tr>
            <td>
                <label for="archi_featured_image_parallax">
                    <?php _e('Effet Parallax', 'archi-graph'); ?>
                </label><br>
                <select id="archi_featured_image_parallax" name="archi_featured_image_parallax">
                    <option value="none" <?php selected($parallax, 'none'); ?>>
                        <?php _e('Aucun effet', 'archi-graph'); ?>
                    </option>
                    <option value="scroll" <?php selected($parallax, 'scroll'); ?>>
                        <?php _e('Parallax Scroll (transform)', 'archi-graph'); ?>
                    </option>
                    <option value="fixed" <?php selected($parallax, 'fixed'); ?>>
                        <?php _e('Parallax Fixed (background-attachment)', 'archi-graph'); ?>
                    </option>
                    <option value="zoom" <?php selected($parallax, 'zoom'); ?>>
                        <?php _e('Zoom progressif au scroll', 'archi-graph'); ?>
                    </option>
                </select>
                <p class="description">
                    <?php _e('Ajoute un effet parallax à l\'image à la une', 'archi-graph'); ?>
                </p>
            </td>
        </tr>
        ...
        <tr>
            <td>
                <label for="archi_featured_image_overlay_opacity">
                    <?php _e('Opacité de l\'overlay', 'archi-graph'); ?>
                </label><br>
                <input type="range" 
                       id="archi_featured_image_overlay_opacity" 
                       name="archi_featured_image_overlay_opacity" 
                       min="0" 
                       max="1" 
                       step="0.1" 
                       value="<?php echo esc_attr($overlay_opacity); ?>"
                       oninput="this.nextElementSibling.textContent = Math.round(this.value * 100) + '%'">
                <span id="overlay-opacity-value"><?php echo round($overlay_opacity * 100); ?>%</span>
                <p class="description">
                    <?php _e('Intensité de l\'overlay sombre sur l\'image (0-100%)', 'archi-graph'); ?>
                </p>
            </td>
        </tr>
    </table>
...
function archi_save_meta_box_data($post_id) {
    ...
    // Vérifications de sécurité pour la meta box des options d'image à la une
    if (isset($_POST['archi_featured_image_meta_box_nonce']) && 
        wp_verify_nonce($_POST['archi_featured_image_meta_box_nonce'], 'archi_featured_image_meta_box')) {
        ...
        if (isset($_POST['archi_featured_image_parallax'])) {
            $parallax = sanitize_text_field($_POST['archi_featured_image_parallax']);
            $allowed_parallax = ['none', 'scroll', 'fixed', 'zoom'];
            if (in_array($parallax, $allowed_parallax)) {
                update_post_meta($post_id, '_archi_featured_image_parallax', $parallax);
            }
        }
        
        // Sauvegarder l'opacité de l'overlay
        if (isset($_POST['archi_featured_image_overlay_opacity'])) {
            $opacity = floatval($_POST['archi_featured_image_overlay_opacity']);
            if ($opacity >= 0 && $opacity <= 1) {
                update_post_meta($post_id, '_archi_featured_image_overlay_opacity', $opacity);
            }
        }
    }
